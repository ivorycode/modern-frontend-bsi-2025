...
initialze();
console.log(
  `Numberguess Controller: Guess the number -> send a POST to ${BASE_URL}/numberguess`
);

export const getStatus = (req, res) => {
  const context = getContext(req);

  if (!context.latestGuess) {
    res.status(500).send("No guess yet!");
    return;
  }
...
export const guessNumber = (req, res) => {
  const context = getContext(req);

  let newGuess = req.body.number;
  if (!newGuess) {
    res.status(500).send("Not a valid guess!");
    return;
  }

  context.latestGuess = newGuess;
  console.log("Guessing: ", context.latestGuess);
  context.guessCount++;
